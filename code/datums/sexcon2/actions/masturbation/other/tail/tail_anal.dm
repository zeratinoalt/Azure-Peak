...
/datum/sex_action/masturbate/other/tailjob_anal/get_start_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	return span_warning("[user] [do_subtle ? "subtly " : ""]slides [user.p_their()] tail into [target]'s butt!")
...
/datum/sex_action/masturbate/other/tailjob_anal/get_finish_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	return span_warning("[user] [do_subtle ? "subtly " : ""]pulls [user.p_their()] tail out of [target]'s butt.")
...
/datum/sex_action/masturbate/other/tailjob_anal/on_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	user.visible_message(sex_session.spanify_force("[user] [sex_session.get_generic_force_adjective(do_subtle)] prods [target]'s butt with [user.p_their()] tail..."), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
