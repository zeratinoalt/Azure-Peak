...
/datum/sex_action/masturbate/other/magejob_vagina/shows_on_menu(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(user == target)
		return FALSE
...
/datum/sex_action/masturbate/other/magejob_vagina/can_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
	if(!.)
		return FALSE
	if(user == target)
...
/datum/sex_action/masturbate/other/magejob_vagina/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.visible_message(span_warning("[user] starts to [do_subtle ? "subtly " : ""]finger [target]'s cunt with arcyne..."), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/masturbate/other/magejob_vagina/lock_sex_object(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
