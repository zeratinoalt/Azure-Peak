...
/datum/tgui_say/proc/load()
	...
	window.send_message("props", list(
		...
		"scale" = FALSE, //client?.prefs?.read_preference(/datum/preference/toggle/ui_scale),
		"minimumWidth" = minimum_width,
		"minimumHeight" = minimum_height,
		"maxLength" = max_length,
		"spellcheck" = FALSE //client?.prefs?.read_preference(/datum/preference/toggle/tgui_use_spellcheck)
	))
	return TRUE
...
/datum/tgui_say/proc/on_message(type, payload, href_list)
	...
	if(type == "close")
		...
		return TRUE
	if(type == "typing")
		client.mob.display_typing_indicator()
		return TRUE
